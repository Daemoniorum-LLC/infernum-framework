...
| Module | Rust LOC | Sigil LOC | Reduction | Notes |
|--------|----------|-----------|-----------|-------|
| types.rs/sigil | 223 | ~120 | **46%** | Inline defaults, simpler From impls |
| error.rs/sigil | 145 | ~90 | **38%** | Updated: new error variants + helper methods |
| streaming.rs/sigil | 216 | ~140 | **35%** | Morpheme pipes for collect, StreamChunkBuilder |
| sampling.rs/sigil | 193 | ~130 | **33%** | Inline defaults, validation pipes |
| request.rs/sigil | 242 | ~150 | **38%** | Evidentiality markers |
| response.rs/sigil | 153 | ~100 | **35%** | Evidentiality for LLM outputs |
| model.rs/sigil | 388 | ~220 | **43%** | Set operators for arch checks |
| **TOTAL** | **1,560** | **~950** | **39.1%** | |
...
**Streaming Tests:**
- `collect_text.json` - Token stream collection (6 cases)
- `chunk_creation.json` - StreamChunk builder (6 cases)
- `empty_stream.json` - Edge cases for empty streams (3 cases)

**Sampling Tests:**
- `validation.json` - Parameter validation (12 cases)
- `presets.json` - Preset constructors (4 cases)
- `builder.json` - Builder pattern (4 cases)

**Type Tests:**
- `usage.json` - Usage statistics (4 cases)
- `model_id.json` - ModelId type (4 cases)
- `request_id.json` - RequestId type (4 cases)
...
### 2025-12-03: Feature Parity Update (infernum-core 0.1.0)
- Updated error.sigil to match infernum-core 0.1.0 error types (100% faithful port)
  - Added: UnsupportedArchitecture, OutOfMemory, ContextLengthExceeded, Timeout, RateLimited, ModelLoad, Io, Serialization
  - Added: is_retryable(), is_resource_exhaustion(), internal(), backend(), model_load() helper methods
- Added missing test files: empty_stream.json, builder.json, model_id.json, request_id.json
- Updated test runner as standalone JSON validator (no external dependencies)
- Total test cases: 22 → 31 (9 new test cases)
