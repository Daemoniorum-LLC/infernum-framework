...
fn main() -> anyhow::Result<()> {
    ...
    println!("Flash Attention: {}", if use_flash { "ENABLED" } else { "DISABLED" });
...
"#;
    ...
    // Tokenize
    let encoding = tokenizer.encode(long_prompt.as_str(), false)
    ...
    println!("  Prefill: {:.2}s ({:.1} tokens/s)", 
    ...
    println!("  Decode: {:.2}s ({:.1} tokens/s)", decode_time.as_secs_f64(), decode_tps);

    // Summary
    let total_time = prefill_time + decode_time;
    println!("\n{}", "=".repeat(60));
    println!("RESULTS ({}):", if use_flash { "Flash Attention" } else { "Standard Attention" });
    println!("  Prefill ({} tokens): {:.2}s ({:.1} tok/s)", 
             prompt_len, prefill_time.as_secs_f64(),
             prompt_len as f64 / prefill_time.as_secs_f64());
    println!("  Decode ({} tokens):  {:.2}s ({:.1} tok/s)", 
             generated.len(), decode_time.as_secs_f64(), decode_tps);
    println!("  Total time: {:.2}s", total_time.as_secs_f64());
    println!("{}", "=".repeat(60));

    // Show generated text
    let generated_text = tokenizer.decode(&generated, false)
