...
fn main() -> Result<()> {
    ...
    let hct_dir = PathBuf::from("/home/crook/.cache/infernum/models/hct/Qwen--Qwen2.5-7B-HoloTensor-v2");

    if !hct_dir.exists() {
        println!("ERROR: 7B HoloTensor model not found at: {}", hct_dir.display());
        return Ok(());
    }
    ...
    // Quality assessment
    let has_garbage = generated_text.chars().any(|c| !c.is_ascii() && !c.is_alphanumeric())
