...
use haagenti::holotensor::{HoloTensorReader, HoloTensorDecoder, HOLO_MAGIC};
...
fn main() -> anyhow::Result<()> {
    ...
    eprintln!("  total_fragments: {}", holo_reader.header().total_fragments);
    ...
    for (i, frag) in fragments.iter().take(3).enumerate() {
        eprintln!("Fragment {}: index={}, flags=0x{:04x}, data_len={}", 
                 i, frag.index, frag.flags, frag.data.len());
    }
