...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("CPU LRDF Encoding:");
    {
        ...
        println!("  Total fragment bytes: {} KB",
    ...
    #[cfg(feature = "cuda")]
    {
        ...
        // Calculate quality
        let dot: f64 = data.iter().zip(reconstructed.iter())
            .map(|(a, b)| (*a as f64) * (*b as f64))
            .sum();
        let norm_a: f64 = data.iter().map(|x| (*x as f64).powi(2)).sum::<f64>().sqrt();
        let norm_b: f64 = reconstructed.iter().map(|x| (*x as f64).powi(2)).sum::<f64>().sqrt();
        let similarity = dot / (norm_a * norm_b);

        println!("  Cosine similarity: {:.4}", similarity);
        println!("  Status: {}", if similarity > 0.9 { "PASS" } else { "FAIL" });
    }
