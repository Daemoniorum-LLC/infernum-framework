...
fn main() -> Result<()> {
    ...
    let st_embed_floats: Vec<f32> = st_embed_data.chunks_exact(2)
    ...
    println!("Embedding lookup cosine similarity: {:.6}", cosine_similarity(&orig_flat, &hct_flat));
    println!("Original embedding stats: mean={:.6}, std={:.6}",
             orig_flat.iter().sum::<f32>() / orig_flat.len() as f32,
             (orig_flat.iter().map(|x| x.powi(2)).sum::<f32>() / orig_flat.len() as f32).sqrt());
    println!("HCT embedding stats: mean={:.6}, std={:.6}",
    ...
    println!("Logits cosine similarity (orig hidden, orig/hct lmhead): {:.6}",
    ...
    println!("  orig_lmhead(hct_hidden) stats: mean={:.4}, min={:.4}, max={:.4}",
             orig_logits2_flat.iter().sum::<f32>() / orig_logits2_flat.len() as f32,
             orig_logits2_flat.iter().cloned().fold(f32::INFINITY, f32::min),
             orig_logits2_flat.iter().cloned().fold(f32::NEG_INFINITY, f32::max));
    println!("  hct_lmhead(hct_hidden) stats: mean={:.4}, min={:.4}, max={:.4}",
             hct_logits2_flat.iter().sum::<f32>() / hct_logits2_flat.len() as f32,
             hct_logits2_flat.iter().cloned().fold(f32::INFINITY, f32::min),
             hct_logits2_flat.iter().cloned().fold(f32::NEG_INFINITY, f32::max));
    println!("Cosine similarity (hct_hidden): {:.6}", cosine_similarity(&orig_logits2_flat, &hct_logits2_flat));
    ...
    println!("Random hidden -> logits cosine similarity: {:.6}", cosine_similarity(&orig_rand_flat, &hct_rand_flat));
