//! Test original safetensors vs 60% compressed - quality comparison
...
fn main() -> Result<()> {
    println!("=== 60% Retention Quality Test ===\n");

    let safetensors_path = Path::new("/home/crook/models/llama-3.2-1b/model.safetensors");
    let compressed_dir = Path::new("/home/crook/models/llama-3.2-1b-hct-60pct");
    ...
    // Load 60% compressed hybrid
    println!("\nLoading 60% compressed hybrid...");
    let start = Instant::now();
    ...
    let (compressed_tokens, compressed_logits) = run_inference(compressed_tensors, "60% Retention (hybrid)", &device, dtype)?;
