...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let model_dir = if args.len() > 1 && args[1] == "--model" && args.len() > 2 {
        ...
        "/home/crook/dev2/workspace/nyx/infernum/infernum-complete/test_models/smollm2-135m-int4".to_string()
    };
    ...
    println!("Memory used: {:.2} MB", weights.memory_used as f64 / 1024.0 / 1024.0);
    ...
    for (seq_len, label) in test_sequences {
        ...
        println!("{:25} | {:7.2} ms | {:8.0} tok/s", label, avg_ms, tokens_per_sec);
    }
