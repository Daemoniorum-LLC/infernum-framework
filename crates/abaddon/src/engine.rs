...
impl Engine {
    ...
    pub async fn new(config: EngineConfig) -> Result<Self> {
        ...
        // Determine dtype - use F32 for CUDA (GTX 1060 doesn't support FP16)
        let dtype = if device.is_cuda() {
            DType::F32  // CUDA needs F32 for older GPUs without tensor cores
        } else {
            DType::F16  // CPU/Metal can use F16
        };
